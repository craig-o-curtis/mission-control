...
class TestAdminGetAllTasks:
    def test_admin_gets_all_tasks(self, admin_client: TestClient) -> None:
    ...
    def test_admin_gets_all_tasks_pagination(self, admin_client: TestClient) -> None:
        """Admin can paginate through all tasks."""
        response = admin_client.get("/admin/tasks?skip=0&limit=1")
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)

    def test_non_admin_cannot_get_all_tasks(self, api_client: TestClient) -> None:
        """Non-admin user gets 403 on admin tasks endpoint."""
        response = api_client.get("/admin/tasks")
        assert response.status_code == 403


class TestAdminGetTask:
    def test_admin_gets_task_by_id(self, admin_client: TestClient) -> None:
        """Admin can retrieve a single task by ID."""
        response = admin_client.get("/admin/tasks/1")
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == 1

    def test_admin_gets_task_not_found(self, admin_client: TestClient) -> None:
        """Admin gets 404 for non-existent task."""
        response = admin_client.get("/admin/tasks/999")
        assert response.status_code == 404
        assert response.json()["detail"] == "Task not found"

    def test_non_admin_cannot_get_task(self, api_client: TestClient) -> None:
        """Non-admin user gets 403 on admin task endpoint."""
        response = api_client.get("/admin/tasks/1")
        assert response.status_code == 403


class TestAdminCreateTask:
...
class TestAdminDeleteTask:
    def test_admin_deletes_task(self, admin_client: TestClient) -> None:
        """Admin can delete any task."""
        response = admin_client.delete("/admin/tasks/1")
        assert response.status_code == 204
        assert response.content == b""

    def test_admin_deletes_task_not_found(self, admin_client: TestClient) -> None:
        """Admin gets 404 when deleting non-existent task."""
        response = admin_client.delete("/admin/tasks/999")
        assert response.status_code == 404
        assert response.json()["detail"] == "Task not found"

    def test_non_admin_cannot_delete_task(self, api_client: TestClient) -> None:
        """Non-admin user gets 403 on admin delete task."""
        response = api_client.delete("/admin/tasks/1")
        assert response.status_code == 403
