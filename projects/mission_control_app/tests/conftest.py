...
import pymysql
import pytest
from checklists_api.app import app
from checklists_api.database import Base
from checklists_api.dependencies.db_dep import DbDep, get_db
from checklists_api.dependencies.user_dep import get_current_user
...
@pytest.fixture
def fake_checklist_item(fake_user: User) -> ChecklistItem:
    """Create a default checklist item owned by fake_user."""
    return ChecklistItem(
        checklist_item="Test Checklist Item",
...
@pytest.fixture
def fake_admin_checklist_item(fake_admin_user: User) -> ChecklistItem:
    """Create a default checklist item owned by fake_admin_user."""
    return ChecklistItem(
        checklist_item="Admin Test Checklist Item",
        description="A checklist item for admin testing.",
        criticality=2,
        executed=False,
...
@pytest.fixture
def fake_second_user_checklist_item(fake_second_user: User) -> ChecklistItem:
    """Create a checklist item owned by fake_second_user for isolation tests."""
    return ChecklistItem(
        checklist_item="Other User's Checklist Item",
...
def _ensure_mysql_database() -> None:
    ...
    try:
        db_name = parsed.path.lstrip("/") or os.getenv(
            "TEST_MYSQL_DB", "checklists_application_database_test"
        )
...
def _ensure_postgres_database() -> None:
    ...
    db_name = parsed.path.lstrip("/") or os.getenv(
        "TEST_POSTGRES_DB", "ChecklistsApplicationDatabase_test"
    )
...
@pytest.fixture
def api_client(
    fake_user: User, fake_checklist_item: ChecklistItem, test_session
) -> Generator[TestClient]:
    """Client authenticated as a regular user with a default checklist item in DB."""
    test_session.add(fake_user)
    test_session.flush()
    test_session.add(fake_checklist_item)
    test_session.commit()
...
@pytest.fixture
def admin_client(
    fake_admin_user: User, fake_admin_checklist_item: ChecklistItem, test_session
) -> Generator[TestClient]:
    """Client authenticated as admin with a default checklist item in DB."""
    test_session.add(fake_admin_user)
    test_session.flush()
    test_session.add(fake_admin_checklist_item)
    test_session.commit()
...
def isolation_client(
    ...
    fake_second_user_checklist_item: ChecklistItem,
    test_session,
) -> Generator[TestClient]:
    """Client authenticated as user 1, but DB has a checklist item owned by user 2."""
    test_session.add(fake_second_user)
    test_session.flush()
    test_session.add(fake_second_user_checklist_item)
    test_session.commit()
