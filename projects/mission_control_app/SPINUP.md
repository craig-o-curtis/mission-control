...
All commands run from the `projects/mission_control_app/` directory. From the monorepo root:

```bash
cd projects/mission_control_app
```
...
The app's lifespan calls `init_db()`, which auto-creates the `users` and `checklist_items` tables if they don't exist.
...
In the terminal running `uv run checklists-api`:
