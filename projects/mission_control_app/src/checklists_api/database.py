...
def _ensure_seeded_column() -> None:
    """Add the ``seeded`` column to ``checklist_items`` if a previous deployment
    created the table before this column existed.
    ...
    if "checklist_items" not in inspector.get_table_names():
        return
    existing = {col["name"] for col in inspector.get_columns("checklist_items")}
    if "seeded" in existing:
        return
    with engine.begin() as conn:
        conn.execute(
            text(
                "ALTER TABLE checklist_items ADD COLUMN seeded "
