...
- Database: `checklists_application_database`
...
```bash
uv run checklists-api
```

The app's `init_db()` auto-creates the `users` and `checklist_items` tables on startup.
...
2. Create the database:
   ```bash
   mysql -u root -p -e "CREATE DATABASE IF NOT EXISTS checklists_application_database;"
   ```
...
> **Note:** Tests use a separate database (`checklists_application_database_test`). Create it first:

```bash
docker compose -f docker-compose.mysql.yml exec mysql mysql -u root -p12345678 -e "CREATE DATABASE IF NOT EXISTS checklists_application_database_test;"
```

Then run:

```bash
TEST_DATABASE_URL=mysql+pymysql://root:12345678@localhost:3306/checklists_application_database_test uv run pytest projects/mission_control_app/tests/ -v
```
...
| `MYSQL_DATABASE`           | `checklists_application_database` | Database name           |
...
| `POSTGRES_DB`              | `ChecklistsApplicationDatabase`   | Database name           |
