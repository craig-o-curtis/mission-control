...
```bash
docker compose -f docker-compose.postgres.yml exec db pg_isready -U postgres -d ChecklistsApplicationDatabase
```
...
- **General:** Name: `Checklists DB`
- **Connection:**
  - Host: `db`
  - Port: `5432`
  - Maintenance database: `ChecklistsApplicationDatabase`
  - Username: `postgres`
...
2. Create the database:
   ```bash
   createdb ChecklistsApplicationDatabase
   ```
...
> **Note:** Tests use a separate database (`ChecklistsApplicationDatabase_test`). Create it first:

```bash
docker compose -f docker-compose.postgres.yml exec db psql -U postgres -c "CREATE DATABASE \"ChecklistsApplicationDatabase_test\";"
```

Then run:

```bash
TEST_DATABASE_URL=postgresql+psycopg://postgres:12345678@localhost:5432/ChecklistsApplicationDatabase_test uv run pytest projects/mission_control_app/tests/ -v
```
