...
<section class="mx-auto max-w-5xl">
  <div class="mb-6 flex items-end gap-4">
    <div class="lcars-curve-left bg-lcars-blue px-6 py-3">
      <h1 class="text-xl font-bold tracking-widest text-black">
        CHECKLIST OPS
      </h1>
    </div>
    ...
    {#if token}
      <div class="ml-auto flex gap-3">
        <button class="lcars-btn bg-lcars-red text-black" onclick={reset}
          >Reset Data</button
        >
        <button class="lcars-btn bg-lcars-purple text-black" onclick={logout}
          >Log Out</button
        >
      </div>
    {/if}
  ...
  {#if !token}
    <div class="lcars-card border-lcars-blue bg-lcars-panel">
      ...
      <p class="mt-4 text-sm text-gray-500">
        ...
        <code class="text-lcars-yellow">ADMIN_USER</code> /
        <code class="text-lcars-yellow">ADMIN_PASSWORD</code>
  ...
  {:else}
    <div class="lcars-card mb-8 border-lcars-teal bg-lcars-panel">
      <form class="grid gap-3 sm:grid-cols-3" onsubmit={submit}>
        ...
        <div class="flex gap-3 sm:col-span-3">
          ...
          {#if editingId !== null}
            <button
              class="lcars-btn bg-lcars-purple text-white"
              onclick={cancelEdit}
            >
              Cancel
            </button>
    ...
    {:else}
      <div
        class="lcars-curve-left overflow-hidden border-l-8 border-lcars-blue bg-lcars-panel"
      >
        {#each checklistItems as item (item.id)}
          ...
          >
            <div class="flex items-center gap-4">
              <div
                class="lcars-pill bg-lcars-orange px-3 py-1 text-xs font-bold text-black"
              >
                #{item.id}
              </div>
              <span
