...
<section class="mx-auto max-w-5xl">
	<div class="mb-6 flex items-end gap-4">
		<div class="lcars-curve-left bg-lcars-orange px-6 py-3">
			<h1 class="text-xl font-bold tracking-widest text-black">MISSION CONTROL</h1>
		</div>
		<div class="lcars-curve-right bg-lcars-blue px-4 py-2">
			<span class="text-sm tracking-wider text-white">MISSION DATABASE</span>
		</div>
		<button
			class="lcars-btn ml-auto bg-lcars-red text-white"
			onclick={reset}
		>
			Reset Data
		</button>
	...
	<div class="lcars-card mb-8 border-lcars-purple bg-lcars-panel">
		<form class="grid gap-3 sm:grid-cols-3" onsubmit={submit}>
			<input class="lcars-input" placeholder="Mission name" bind:value={mission_name} required />
			<input class="lcars-input" placeholder="Commander" bind:value={commander} required />
			<input class="lcars-input" placeholder="Mission type" bind:value={mission_type} required />
			...
			<input
				...
				bind:value={priority}
			/>
			<input class="lcars-input" placeholder="Launch date" bind:value={launch_date} />
			<input
			...
			<div class="flex gap-3 sm:col-span-3">
				...
				{#if editingId !== null}
					<button class="lcars-btn bg-lcars-blue text-white" onclick={cancelEdit}>
						Cancel
					</button>
				{/if}
			</div>
	...
	{:else}
		<div class="lcars-curve-left overflow-hidden border-l-8 border-lcars-orange bg-lcars-panel">
			{#each missions as mission (mission.id)}
				<div class="flex items-center justify-between border-b border-gray-800 px-6 py-4">
					<div class="flex items-center gap-4">
						<div class="lcars-pill bg-lcars-blue px-3 py-1 text-xs font-bold text-white">
							#{mission.id}
						</div>
						<span class="font-medium text-lcars-cream">{mission.mission_name}</span>
						<span class="text-gray-500">
							— {mission.commander} ({mission.mission_type}) · P{mission.priority ?? '—'}
						</span>
						<span class="lcars-pill px-2 py-0.5 text-xs {phaseClass(mission.phase)}">
							{mission.phase ?? '—'}
						</span>
						{#if mission.seeded}
					...
					<div class="flex gap-3 text-sm">
						<button class="text-lcars-teal hover:underline" onclick={() => startEdit(mission)}>
							Edit
						</button>
						<button class="text-lcars-red hover:underline" onclick={() => remove(mission.id)}>
							Delete
						</button>
					</div>
				</div>
